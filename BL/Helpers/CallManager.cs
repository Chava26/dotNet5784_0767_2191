...
internal class CallManager
{
    ...
    internal static BO.Call CreateBoCall(DO.Call call, List<CallAssignInList> callAssignments)
    {
        return new BO.Call
        {
            ...
            Status = Tools.CalculateCallStatus(call),
    ...
    internal static void PeriodicCallsUpdates(DateTime oldClock, DateTime newClock)
    {
        ...
        lock (AdminManager.BlMutex) //stage 7
        {
            ...
            var filter = new Func<DO.Call, bool>(c => c.MaxFinishTime.HasValue && c.MaxFinishTime < now);

            expiredCalls = s_dal.Call.ReadAll(filter).ToList();
    ...
    static public IEnumerable<BO.OpenCallInList> GetOpenCalls(int volunteerId, BO.CallType? callType, BO.CallField? sortField)
    {
        ...
        lock (AdminManager.BlMutex)
        {
            ...
            .Where(c =>
            (Tools.CalculateCallStatus(c) == BO.CallStatus.Open || Tools.CalculateCallStatus(c) == BO.CallStatus.OpenRisk))
            .Select(c => new BO.OpenCallInList
            {
                ...
                DistanceFromVolunteer = Tools.CalculateDistance(volunteerLat, volunteerLon, c.Latitude ?? 0, c.Longitude ?? 0)
            });
